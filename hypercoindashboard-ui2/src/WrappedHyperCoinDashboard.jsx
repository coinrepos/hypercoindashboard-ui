...
const WrappedHyperCoinDashboard = () => {
  ...
  const handleMintLuckyCoin = async () => {
    try {
      const res = await axios.post(`${API_BASE_URL}/mintLuckyCoin`, {
        to: address,
        amount: convertedAmount
      });
      setResponse('MintLuckyCoin: ' + res.data.tx_hash);
    } catch (err) {
      setResponse('MintLuckyCoin error: ' + err.message);
    }
  };

  const handleDeployDAO = async () => {
    try {
      const res = await axios.post(`${API_BASE_URL}/deployDAO`);
      setResponse('DeployDAO: ' + res.data.tx_hash);
    } catch (err) {
      setResponse('DeployDAO error: ' + err.message);
    }
  };
  ...
  return (
    <div style={{ padding: '2rem', background: '#0f172a', color: '#fff' }}>
      ...
      <h2>Admin Panel</h2>
      <button onClick={handleMintLuckyCoin}>💰 MintLuckyCoin</button>
      <button onClick={handleDeployDAO} style={{ marginLeft: '10px' }}>🏛 Deploy DAO</button>
